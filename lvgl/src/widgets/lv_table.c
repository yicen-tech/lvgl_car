...
void lv_table_set_cell_align(lv_obj_t * obj, uint16_t row, uint16_t col, lv_text_align_t align)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
    ...
    /* 如果单元格为空，分配内存 */
    if(is_cell_empty(table->cell_data[cell])) {
        table->cell_data[cell] = lv_mem_alloc(2); /* +1: 控制字节; +1: '\0' 结束符 */
        LV_ASSERT_MALLOC(table->cell_data[cell]);
        if(table->cell_data[cell] == NULL) return;

        table->cell_data[cell][0] = 0; // 初始化控制字节
        table->cell_data[cell][1] = '\0'; // 初始化为空字符串
    }

    /* 设置对齐方式 */
    switch(align) {
        case LV_TEXT_ALIGN_LEFT:
            table->cell_data[cell][0] &= ~LV_TABLE_CELL_CTRL_TEXT_CROP; // 清除其他对齐方式
            break;
        case LV_TEXT_ALIGN_CENTER:
            table->cell_data[cell][0] |= LV_TABLE_CELL_CTRL_TEXT_CROP; // 设置居中对齐
            break;
        case LV_TEXT_ALIGN_RIGHT:
            table->cell_data[cell][0] |= LV_TABLE_CELL_CTRL_TEXT_CROP; // 设置右对齐
            break;
        default:
            LV_LOG_WARN("Invalid alignment type");
            return;
    }
